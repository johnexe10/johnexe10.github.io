var main = (function($) { var _ = {
					...
					// Slide.
						...
						// Create elements.
	 					...
	 					// Image.
							...
							s.$slideImage.css('background-image','linear-gradient(140deg, '+$thumbnail.data('bgcolor')+')')
						// Banner.
							s.$slideBanner = s.$slide.children('.banner');

							// Set size 
								s.$slideBanner
								.css('width', ($thumbnail.data('width')))
								.css('height', ($thumbnail.data('height')));
							// Set iframes
								s.$slideBanner.append('<iframe width='+$thumbnail.data('width')+' height='+$thumbnail.data('height')+' src='+$thumbnail.data('banner')+' frameborder="0" allowfullscreen=""></iframe>');
	...
	switchTo: function(index, noHide) {
		...
		// Deactivate old slide (if there is one).
			...
			_.oldColor = (oldSlide !== null ? oldSlide.$slideImage.css('background-image') : 'black'); 
			_.newColor = newSlide.$slideImage.css('background-image'); 
			if (oldSlide) {
				// Thumbnail.
					oldSlide.$parent
						.removeClass('active');
			...
			// Slide.
				var f = function() {
					...
					// Attach new slide.
						...
						gsap.fromTo(newSlide.$slideImage[0],{backgroundImage:_.oldColor},{duration:1,backgroundImage:_.newColor,ease:"power1.in"})
								...
								// Wait for it to load.
									...
									//window.setTimeout(function() {
										...
										// Mark as loaded.
											...
											// newSlide.$slide.removeClass('loading'); temp disable. always on for now
